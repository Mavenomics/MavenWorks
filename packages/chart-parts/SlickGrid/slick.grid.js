...
(function ($) {
    ...
    function SlickGrid (container, data, columns, options) {
        ...
        function init () {
            ...
            // column grouping
            $paneGrouping = $("<div class='slick-pane slick-pane-grouping' tabIndex='0'/>").appendTo($container);
        ...
        function updateCanvasWidth (forceColumnWidthsUpdate) {
            ...
            if (widthChanged || options.frozenColumn > -1 || hasFrozenRows) {
                ...
                if (options.frozenColumn > -1) {
                    ...
                    $paneTopL.width(canvasWidthL - 1)
                        .css("borderRight", "1px solid #696969");
        ...
        function createColumnHeaders () {
            ...
            // TODO grouping needs to be done here, before the .height() call
            ...
            var maxGrouping = Math.max.apply(null, columns.map(function (c) { return c.columnGroups != null ? c.columnGroups.length : 0; }));
            ...
            for (var i = 0; i < columns.length; i++) {
                ...
                var headerHeight = 18;
                var header = $("<div class='slick-header-columngroup ui-state-default slick-header-column' />")
                    .data("column", m)
                    .width(m.width - headerColumnWidthDiff)
                    .height((maxGrouping + 1) * headerHeight)
                    //.css("lineHeight", ((maxGrouping + 1) * headerHeight) + "px")
                    .attr("id", "" + uid + m.id)
                    .attr("title", m.toolTip || "")
                    .addClass(m.headerCssClass || "")
                    .appendTo($headerTarget);

                if (m.width == 0 || m.collapsed)
                ...
                for (var j = 0; j < maxGrouping; j++) {
                    row = $("<tr></tr>")
                        .appendTo(table);

                    dt = $("<td></td>")
                        .height(headerHeight)
                        .width(m.id === "path" ? "" : m.width - headerColumnWidthDiff)
                        .appendTo(row);

                    var group = m.columnGroups != null && j < m.columnGroups.length ? m.columnGroups[j] : null;
                    if (group != null) {
                        var previous = i > 0 && columns[i - 1].columnGroups != null ? columns[i - 1].columnGroups : null;
                        var next = i + 1 < columns.length ? columns[i + 1].columnGroups : null;
                        var isStart = previous == null || j >= previous.length || previous[j] != group;

                        row.addClass("column-group-level" + j);

                        if (isStart) {
                            $("<span class='toggle " + (columns[i + 1] && columns[i + 1].collapsed ? "expand" : "collapse") + "'></span>")
                                .css({
                                    float: "left",
                                    paddingRight: 5
                                })
        ...
        function setupColumnSort () {
            $headers.click(function (e) {
                ...
                if (column.sortable) {
                    ...
                    // Don't set the sort columns here, do it manually when we actually sort the data
                    //setSortColumns(sortColumns);
        ...
        function getColumnCssRules (idx) {
            ...
            if (!stylesheet) {
                ...
                for (i = 0; i < sheets.length; i++) {
                    if ((sheets[i].sheet.ownerNode || sheets[i].sheet.owningElement) == $style[0]) {
                        stylesheet = sheets[i].sheet;
                        break;
                    }
                }
        ...
        function getFormatters (row, column) {
            return column.formatters || [getFormatter(row, column)];
        }
        ...
        function appendCellHtml (stringArray, row, cell, colspan, item) {
            ...
            if (item) {
                cellCss += (item.cssClass ? " " + item.cssClass : "");
            }
            ...
            var formatted = [];
            if (item) {
                if (item["cssClass" + cell] !== undefined) {
                    cellCss += " " + item["cssClass" + cell];
                }

                var value = getDataItemValueForColumn(item, columnDef);
                getFormatters(row, columnDef).forEach(function (formatter) {
                    var formattedValue = formatter(row, cell, value, columnDef, item, data);
                    formatted.push(formattedValue);
                });

                cssStyle = item["cssStyle" + cell] || "";
            }

            if (columnDef.id === "ROWSELECTOR") {
                stringArray.push("<div class='" + cellCss + " row-selector'>");
                stringArray.push(item.rowNumber + 1);
            }
            else {
                stringArray.push("<div class='" + cellCss + "' style='" + cssStyle + "'>");
        ...
        function resizeCanvas () {
            ...
            groupPanelH = $paneGrouping.height();

            $paneHeaderL.css("top", groupPanelH);
            $paneHeaderR.css("top", groupPanelH);
            ...
            viewportTopH = paneTopH - topPanelH - headerRowH - groupPanelH;
            ...
            $paneTopL.css({
                'top': $paneHeaderL.height() + groupPanelH, 'height': paneTopH
            });
            ...
            if (options.frozenColumn > -1) {
                $paneTopR.css({
                    //'top': $paneHeaderL.height() + groupPanelH, 'height': paneTopH
                    'top': $paneHeaderR.height() + groupPanelH, 'height': paneTopH
                });
        ...
        function cleanUpAndRenderCells (range) {
            ...
            for (var row = range.top, btm = range.bottom; row <= btm; row++) {
                ...
                for (var i = 0, ii = columns.length; i < ii; i++) {
                    ...
                    if (columns[i].width == 0 || columns[i].collapsed)
                        continue;
                    // Already rendered.
        ...
        function handleMouseWheel (event, delta, deltaX, deltaY) {
            scrollTop = Math.max(0, $viewportScrollContainerY[0].scrollTop - (deltaY * (options.rowHeight * options.linesPerScroll)));
        ...
        function asyncPostProcessRows() {
          ...
          while (postProcessFromRow <= postProcessToRow) {
            ...
            var startRowTime = performance.now();
            ...
            for (var columnIdx in cacheEntry.cellNodesByColumnIdx) {
              ...
              if (m.asyncPostRender && !postProcessedRows[row][columnIdx]) {
                ...
                if (node) {
                  ...
                  lastRenderTime = performance.now() - startRowTime;
                }
            ...
            var frameTime = performance.now() - startFrame;
            if (frameTime + lastRenderTime > 15) {
              h_postrender = setTimeout(asyncPostProcessRows, options.asyncPostRenderDelay);
              return;
            }
          }
        ...
        function handleKeyDown (e) {
                ...
                if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
                    if (e.which == 27) {
                        if (!getEditorLock().isActive()) {
                            return; // no editing mode to cancel, allow bubbling and default processing (exit without
            ...
            if (handled) {
                // the event has been handled so don't let parent element (bubbling/propagation) or browser (default)
                // handle it
                e.stopPropagation();
                e.preventDefault();
                try {
                    e.originalEvent.keyCode = 0; // prevent default behaviour for special keys in IE browsers (F3, F5,
                                                 // etc.)
                }
                    // ignore exceptions - setting the original event's keycode throws
                    // access denied exception for "Ctrl" (hitting control key only, nothing else), "Shift" (maybe
                    // others)
                catch (error) {
                }
            }
        ...
        function handleClick (e) {
            ...
            // TODO NICK: do we really want to stop click events if this is the active cell?
        ...
        function handleMouseDown (e) {
            // don't do anything here, allow it to be managed externally
            var cell = getCellFromEvent(e);
            if (cell)
        ...
        function setActiveCellInternal (newCell, opt_editMode, triggerEvent) {
            ...
            if (activeCellChanged) {
                setTimeout(scrollActiveCellIntoView, 2);
        ...
        function scrollRowIntoView (row, doPaging) {
            ...
            var rowAtBottom = (row + 4) * options.rowHeight
                - viewportScrollH
        ...
        function setActiveCell (row, cell, triggerEvent) {
            if (!initialized) {
                return;
            }
            ...
            setActiveCellInternal(getCellNode(row, cell), false, triggerEvent);
        }
